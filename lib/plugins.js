...
function plugins (settings, deviceId) {
  ...
  function notifyConfirmation (tx) {
    ...
    const message = `Your cash is waiting! Go to the Cryptomat and press Redeem within 24 hours. [${timestamp}]`
    const notified = tx.phone ?  notifySMS(tx.phone, message) : notifyEmail(tx.email, message);
  ...
  function notifySMS (phone, message) {
    const rec = {
      sms: {
        toNumber: phone,
        body: message
  ...
  function notifyEmail(toEmail, message) {
    const rec = {
      email: {
        subject: 'Translation almots completed!',
        body: message
      }
    }
    return email.sendMessage({...settings, toEmail}, rec)
  }
  ...
  function getEmailCode (toEmail) {
    const code = argv.mockEmail ? '123': randomCode()
    const rec = {
      email: {
        subject: 'Cryptomat code',
        body: `Your cryptomat code: ${code}`
      }
    }

    return email
      .sendVerificationMessage(settings, rec, 'm-przybylski@hotmail.com')
      .then(() => code)
  }
