...
function addOrUpdateCustomer (req) {
  const customerData = req.body.phone || req.body.email
  const get = req.body.phone ? customers.get : customers.getByEmail
  const config = configManager.unscoped(req.settings.config)

  return get(customerData)
    .then(customer => {
...
function getCustomerWithEmail (req, res, next) {
  const pi = plugins(req.settings, req.deviceId)
  ...
  return pi.getEmailCode(email)
    ...
    .catch(err => {
      if (err.name === 'BadEmailError') throw httpError('Bad emial', 401)
      throw err
    })
    .catch(next)
}
...
app.post('/email_code', getCustomerWithEmail)
